...
const missionSchema = z.object({
  ...
  fromBrandIds: z
    .array(z.string())
    .min(1, "At least one source brand is required"),
  toBrandIds: z
    .array(z.string())
    .min(1, "At least one target brand is required"),
  financialImpact: z.number().min(1, "Financial impact must be greater than 0"),
  ...
  const createMissionMutation = useMutation({
    mutationFn: async (data: any) => {
      const response = await fetch("/api/moderation/missions", {
        ...
        body: JSON.stringify({
          ...data,
          fromBrandIds: selectedFromBrands,
          toBrandIds: selectedToBrands,
        }),
      });
  ...
  const approveSwitchLogMutation = useMutation({
    mutationFn: async ({ id, notes }: { id: string; notes?: string }) => {
      const response = await fetch(
        `/api/moderation/switch-logs/${id}/approve`,
        {
          method: "PUT",
          headers: { "Content-Type": "application/json" },
  ...
  const sidebarItems = [
    {
      ...
      label: "Overview",
      icon: Home,
      count: pendingReports.length + pendingSwitchLogs.length,
    },
    {
      id: "reports",
      label: "Reports",
    ...
    {
      id: "switches",
      label: "Switch Approvals",
      icon: Target,
      count: pendingSwitchLogs.length,
    },
    ...
    },
    {
      ...
      label: "Feedback",
      icon: MessageSquare,
      count: feedbackQuestions.length,
    },
                    ...
                    <div className="flex items-center justify-between">
                      <div>
                        ...
                        <p className="text-2xl font-bold text-blue-600">
                          {missions.filter((m: any) => m.isActive).length}
                        </p>
                      </div>
              ...
              <div className="flex items-center justify-between">
                ...
                <Dialog>
                  ...
                  <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
                    ...
                    <Form {...missionForm}>
                      <form
                        onSubmit={missionForm.handleSubmit((data) =>
                      ...
                      >
                          ...
                          <div className="mt-2 space-y-2">
                            <Select
                              onValueChange={(brandId) =>
                                addBrandToSelection(brandId, "from")
                              }
                            >
                              <SelectTrigger>
                              ...
                              <SelectContent>
                                {brands
                                  .filter(
                                    (brand: any) =>
                                      !selectedFromBrands.includes(brand.id)
                                  )
                                  .map((brand: any) => (
                        ...
                        <div>
                          ...
                          <div className="mt-2 space-y-2">
                            <Select
                              onValueChange={(brandId) =>
                                addBrandToSelection(brandId, "to")
                              }
                            >
                              <SelectTrigger>
                              ...
                              <SelectContent>
                                {brands
                                  .filter(
                                    (brand: any) =>
                                      !selectedToBrands.includes(brand.id)
                                  )
                                  .map((brand: any) => (
                                    <SelectItem key={brand.id} value={brand.id}>
                                      {brand.name}
                                ...
                                return (
                                  ...
                                  >
                                    ...
                                    <Button
                                      ...
                                      className="h-4 w-4 p-0"
                                      onClick={() =>
                                        removeBrandFromSelection(brandId, "to")
                                      }
                                    >
                                      <X className="w-3 h-3" />
                                    </Button>
                ...
                <Card>
                  <CardContent className="text-center py-12">
                    ...
                    <h3 className="text-xl font-semibold mb-2">
                      All Switch Logs Reviewed!
                    </h3>
                    <p className="text-muted-foreground">
                      No pending switch logs require approval.
                    </p>
                  </CardContent>
                </Card>
              ) : (
                      ...
                      <CardContent className="p-6">
                          ...
                          <div className="flex-1">
                            <div className="flex items-center gap-2 mb-2">
                              ...
                              <h3 className="font-semibold">
                                Switch from {log.fromBrand?.name} to{" "}
                                {log.toBrand?.name}
                              </h3>
                              <Badge
                                variant="secondary"
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm mb-4">
                              <div>
                                ...
                                <div>
                                  @{log.user?.handle} (Level {log.user?.level})
                                </div>
                              </div>
                              <div>
                                <span className="font-medium text-muted-foreground">
                        ...
                        <div className="flex gap-2 pt-4 border-t">
                          <Button
                            size="sm"
                            onClick={() =>
                              approveSwitchLogMutation.mutate({
                                id: log.switchLog.id,
                              })
                            }
                          ...
                          <Button
                            ...
                            onClick={() => {
                              ...
                              if (notes) {
                                rejectSwitchLogMutation.mutate({
                                  id: log.switchLog.id,
                                  notes,
                                });
                              }
