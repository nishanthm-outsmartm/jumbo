...
export function MessagingSystem({ userId }: MessagingSystemProps) {
  ...
  const { data: messages = [] } = useQuery({
    ...
    queryFn: () =>
      authenticatedApiRequest("GET", `/api/messages/${user?.id}`).then((res) =>
        res.json()
      ),
    enabled: !!user?.id,
  });
  ...
  const { data: unreadMessages = [] } = useQuery({
    ...
    queryFn: () =>
      authenticatedApiRequest("GET", `/api/messages/${user?.id}/unread`).then(
        (res) => res.json()
      ),
    enabled: !!user?.id,
  });
  ...
  const sendMessageMutation = useMutation({
    mutationFn: (messageData: any) =>
      authenticatedApiRequest("POST", "/api/messages", messageData),
    onSuccess: () => {
  ...
  const markAsReadMutation = useMutation({
    mutationFn: (messageId: string) =>
      authenticatedApiRequest("PUT", `/api/messages/${messageId}/read`),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["/api/messages"] });
    },
  });
