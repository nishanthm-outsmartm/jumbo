...
export function SwitchPost({ post }: SwitchPostProps) {
  ...
  const userInitials = post.user.handle.substring(0, 2).toUpperCase();
  const gradientClass = `bg-gradient-to-r from-${['blue', 'green', 'purple', 'pink', 'indigo'][post.user.handle.charCodeAt(0) % 5]}-500 to-${['purple', 'blue', 'pink', 'indigo', 'green'][post.user.handle.charCodeAt(1) % 5]}-500`;
      ...
      <CardContent className="p-6">
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center space-x-3">
            ...
            <div>
              <p className="font-medium text-gray-900">{post.user.handle}</p>
