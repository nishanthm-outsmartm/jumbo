...
export function SwitchPost({ post }: SwitchPostProps) {
  ...
  const userInitials =
    post.user && post.user.handle
      ? post.user.handle.substring(0, 2).toUpperCase()
      : "??";
  const gradientClass =
    post.user && post.user.handle
      ? `bg-gradient-to-r from-${
          ["blue", "green", "purple", "pink", "indigo"][
            post.user.handle.charCodeAt(0) % 5
          ]
        }-500 to-${
          ["purple", "blue", "pink", "indigo", "green"][
            post.user.handle.charCodeAt(1) % 5
          ]
        }-500`
      : "bg-gray-300";
          ...
          <div className="flex items-center space-x-3">
            ...
            <div>
              ...
              <p className="text-sm text-gray-500">
                ...
                {post.user?.level != null ? ` • Level ${post.user.level}` : ""}
              </p>
            </div>
          </div>
          <div className="flex items-center space-x-2">
            {post.user && post.user.level >= 5 && (
              <Badge
                variant="secondary"
